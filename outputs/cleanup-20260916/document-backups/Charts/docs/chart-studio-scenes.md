...
更新：2026-09-12。适用于当前工程源码；已发布的旧版 EXE 不会自动获得本次能力。
...
灵均首次按下从身体右侧抬槌位置起播，保留约 90ms 的下砸轨迹；重按混合为 25ms，按住时下击、上挑连续衔接，松开以 0.10 秒混合收完当前一招。生死两侧根据各自发波频率独立变速，最高 1.65 倍，点按与长按共用每秒 2 次敲击上限；过快输入仍照常发波和判定，不积压角色动作。裙摆随同一动画运动。预览按歌曲时间和输入边界推进，暂停冻结，定位时从初态重演，包含限速后继续按住的准确续招时刻。参数及素材约定见[灵均动画说明](lingjun-animation.md)。自定义场景的其他自动动画仍需自行接入歌曲时钟。
