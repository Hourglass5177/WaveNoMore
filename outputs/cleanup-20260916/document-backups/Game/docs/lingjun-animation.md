...
2026-09-14：增加 `walk` 小步前行，1.6 秒左右各一步。正式进行时根据两侧主地景启停，移动与静息混合 0.18 秒；步频固定，不跟随 BPM、发波或镜头晃动。攻击仍完整接管，收招后以 0.15 秒进入当前背景对应的步态或静息。移动中受击采用静息的 1.2 倍局部幅度。设置见 [角色移动参数](planning/08-角色移动.md)。

参考层由视觉主题的连续性标识指定；时间表在背景安排变化时生成，普通背景直接读取分段速度，编排镜头按固定时间格定位速度阈值并二分到微秒。音频时间与角色快照时间沿用首拍、输入及视觉补偿转换，定位跨过启停边界，暂停冻结。角色锚点不移动，腿部、骨盆、持槌和衣带关键帧均保存到派生 Spine 资源。原始素材保留；本轮未构建发行程序。

`run_lingjun_walk_tests.gd` 检查启停、循环、实际步幅与抬脚、受击、收招和定位；裙摆检查新增完整 `walk`，共 2,763 个姿势。`capture_lingjun_states.gd -- --walk-movie` 在正式 s08 背景中采样静息、移动、攻击、移动受击及停止，输出 `build/lingjun/states/walk-frames/`。
...
基础动画使用 Spine 轨道 0，局部受击使用轨道 1 的 `set_additive(true)`。基础动画完整设置受击涉及的骨骼属性，避免重复采样积累偏移。攻击收招结束后，以 0.15 秒混合进入当前歌曲相位的移动或静息，取决于主地景是否滚动。
...
- `tests/integration/stage/run_lingjun_skirt_tests.gd`：攻击、循环、静息、移动、死亡及跨动作混合，共 2,763 个姿势；检查实际 deform、顶点、附件范围、翻折及前膝和前脚的纹理覆盖。
