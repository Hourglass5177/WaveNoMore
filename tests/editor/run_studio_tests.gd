...
func _run() -> void:
	...
	down.pressed = true
	down.position = Vector2(timeline.x_at(4800), timeline.RULER + 32)
	timeline._gui_input(down)
	var motion := InputEventMouseMotion.new()
	motion.position = Vector2(timeline.x_at(5280), timeline.RULER + 32)
	...
	drag(timeline, Vector2(timeline.x_at(960), timeline.RULER + 32), Vector2(timeline.x_at(1200), timeline.RULER + 32))
	check(doc.find_note("n03").tick == 1200 and doc.find_note("n04").tick == 1200 and timeline.selected.size() == 2, "拖动选区成员保留双侧选区")
	doc.undo()
	drag(timeline, Vector2(timeline.x_at(1440), timeline.RULER + 32), Vector2(timeline.x_at(1560), timeline.RULER + 32))
	check(doc.find_note("n05").tick == 1560 and doc.find_note("n05").tick + doc.find_note("n05").duration_ticks == 2400, "Hold 头把手固定尾点")
	doc.undo()
	drag(timeline, Vector2(timeline.x_at(2400), timeline.RULER + 32), Vector2(timeline.x_at(2520), timeline.RULER + 32))
