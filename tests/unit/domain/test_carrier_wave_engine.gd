...
func _test_single_bell_emits_without_tuning_target() -> void:
	...
	_expect(history.size() >= 5, "单独按住生钟一秒也会持续成波")
...
func _test_frequency_change_only_affects_future_emissions() -> void:
	...
	engine.set_frequency(GameplayTypes.Affinity.ZHU, 6.9, 300_000)
	...
	_expect_near(float(matched.get("frequency_hz", 0.0)), 4.35, 0.001, "旧波保留发射时的频率元数据")


func _test_repeated_frequency_changes_preserve_phase() -> void:
	var engine := _engine()
	engine.set_held(GameplayTypes.Affinity.ZHU, true, 0)
	var base_period: int = roundi(1_000_000.0 / 4.35)
	var high_period: int = roundi(1_000_000.0 / 6.9)
	var low_period: int = roundi(1_000_000.0 / 1.8)
	engine.set_frequency(GameplayTypes.Affinity.ZHU, 6.9, 100_000)
	var after_first_change: int = 100_000 + roundi(
		float(base_period - 100_000) / float(base_period) * float(high_period)
	)
	engine.set_frequency(GameplayTypes.Affinity.ZHU, 1.8, 150_000)
...
func _test_two_sources_are_independent() -> void:
	...
	engine.set_frequency(GameplayTypes.Affinity.ZHU, 6.9, 100_000)
	engine.set_frequency(GameplayTypes.Affinity.XUAN, 1.8, 100_000)
	engine.advance_to(1_000_000)
	_expect_near(engine.frequency_hz(GameplayTypes.Affinity.ZHU), 6.9, 0.001, "生钟频率独立保存")
	_expect_near(engine.frequency_hz(GameplayTypes.Affinity.XUAN), 1.8, 0.001, "死钟频率独立保存")
...
func _test_exclusive_advance_respects_boundary() -> void:
	...
	released.set_held(GameplayTypes.Affinity.ZHU, false, 229_885)
	released.advance_to(229_885, true)
...
func _test_authored_su_manifestations_are_deterministic() -> void:
	...
	for stage_id: String in ["s01", "s02"]:
		...
		for manifestation: Dictionary in first:
			_expect(bool(manifestation.get("success", false)), "%s 的 Perfect Replay 能凝成素音" % stage_id)
			_expect(not Array(manifestation.get("points", [])).is_empty(), "%s 的素音坐标来自真实加强纹" % stage_id)
