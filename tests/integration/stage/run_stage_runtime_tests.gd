...
func _test_presentation_timing_cues() -> void:
	...
	_expect(float(head_state["head_alpha"]) > 0.0 and is_zero_approx(float(head_state["body_reveal"])), "Hold reveals its head before its body")
	hold.set_approach_progress(0.50)
	var body_state: Dictionary = hold.visual_state_snapshot()
	_expect(float(body_state["body_reveal"]) > 0.0 and is_zero_approx(float(body_state["tail_alpha"])), "Hold grows its body before its tail")
	hold.set_approach_progress(0.90)
	_expect(float(hold.visual_state_snapshot()["tail_alpha"]) > 0.0, "Hold reveals its tail last")
