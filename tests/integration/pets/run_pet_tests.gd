extends SceneTree
## 使用现行 A/B 输入与真实关卡装配，覆盖技能、伤害时序、装备和骨骼视图。
var failures := 0
var checks := 0
...
func _test_hold() -> void:
	for advanced in [false, true]:
		...
		check(missed.health_engine.soul_fire == 100, "Hold 头部抵达不单独扣血")
		missed.advance_to(arrival + 125000)
		check(missed.health_engine.soul_fire == 98 and missed.damages[0].timestamp_us == arrival + 125000, "第一单位身体抵达后扣两点")
		...
		for gap in [119999, 120000, 120001]:
			...
			if gap > 120000:
				check(s.damages.size() > 1 and s.damages[0].timestamp_us > 1220001 + s.wave_engine.post_cue_travel_us(0), "断持后剩余身体逐段抵达，不在失败时立即扣整条")
	...
	for shared in [false, true]:
		...
		check(damaged.damages.size() == (4 if shared else 8), "两界同刻身体按伤害组与分段编号去重")
		check(damaged.health_engine.soul_fire == (92 if shared else 84), "逐段二点伤害减免后仍四舍五入为二点")
...
func _test_time() -> void:
	...
	check(early_death.health_engine.failed and early_death.judgments.all(func(record: JudgmentRecord) -> bool: return record.finalized_at_us <= early_death.damages[-1].timestamp_us), "晚输入跨过致死身体抵达后不再参与判定")
...
func _test_flow() -> void:
	...
	await process_frame
	check(modal.entries.size() == 3 and modal.get_node("%Actor").get_child_count() == 1, "菜单可浏览三只随从，每次展示当前一只")
	await screenshot("pets-menu")
	save.data = save.default_data()
	modal._show_pet()
	await process_frame
	check(root.gui_get_focus_owner() == modal.get_node("%Preview") and modal.get_node("%Equip").disabled, "全未解锁时仍可用键盘和手柄预览，不能装备")
	...
	check(scene._pet_views.size() == 2, "两界各一处骨骼表现")
	...
	check(scene._pet_views[0].events.is_empty(), "漏按与进阶尾部提档均不触发技能")
	...
	check(core.damages.is_empty() and scene._pet_views[0].events.is_empty(), "定位重建清除旧伤害与技能反馈")
	for time in range(0, 1200001, 10000):
		scene.stage_session.advance_preview(time)
