...
static func to_tuning_control(event: PhysicalInputEvent, deadzone: float = 0.2) -> Dictionary:
	## 设计画布方向的无量纲速度控制；使用本局规则的径向死区，不保存设备状态。
	...
	if radius > deadzone:
		control_vector = raw_vector / radius * clampf((radius - deadzone) / (1.0 - deadzone), 0.0, 1.0)
	return {
		"exists": true,
