...
func build_waveform(path: String) -> void:
	...
	if _wave_thread != null:
		if path != _wave_path: _wave_cancel[0] = true
		return
	_wave_path = path
	_wave_cancel = [false]
	_wave_thread = Thread.new()
	_wave_thread.start(StudioAudio.decode_peaks.bind(path, _wave_cancel))
