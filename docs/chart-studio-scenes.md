...
这些是 JSON v2 的可选表现字段，不增加格式版本。每个难度独立保存，新谱使用 `s02` 且演出关闭。`scene_id` 存在时采用场景；只有 `theme_id` 的旧谱仍按旧主题装配，不附加背景或演出。用户明确选择场景后移除旧 `theme_id`，保留配色与开关。导入能够匹配目录中同一源入口的旧关卡时，会关联场景并关闭演出，原有事件迁移范围不扩展。
