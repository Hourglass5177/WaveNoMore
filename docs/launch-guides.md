...
正式游戏每次启动依次显示：MEMO 工作室 → 耳机提示 → 操作说明 → 标题等待页 → 任意键唤醒后播放 PV → 选关第一关。操作说明完整显示后，确认键或“继续”按钮进入标题页；输入不会穿透开屏。返回主菜单直接显示菜单，不重复显示引导与 PV。写谱器与外部试玩入口保持原有启动方式。

## 开场 PV

`src/app/intro_video_screen.gd` 使用 Godot VideoStreamPlayer 播放 `assets/pv.ogv`，保留原声，等比显示并留黑边。首次唤醒不再展开主菜单，淡出标题后以 0.45 秒淡入视频。右下角“跳过”使用操作说明同款汇文字体、骨白颜色和动态手柄键帽；确认键、返回键或点击按钮可跳过，唤醒键须先松开。自然播完与跳过共用 0.3 秒淡出，随后选关以 0.4 秒淡入并选中第一张关卡，不自动开局。

PV 期间菜单环境音沿路由淡出，视频原声使用 Music 总线。视频及导入文件仍被 Git 忽略，队友需自行复制到 `Game/assets/pv.ogv`；缺少素材时跳过 PV，应用仍可进入选关。游戏导出沿用全部资源模式，构建机上的该素材会随包导出。

验证：`tests/integration/app_flow/run_intro_video_tests.gd` 检查实际播放、长按防穿透、跳过、默认选关及同进程返回；加 `-- --natural` 检查真实视频自然结束路径。播放器接口遵循 [Godot VideoStreamPlayer 文档](https://docs.godotengine.org/en/stable/classes/class_videostreamplayer.html)。
