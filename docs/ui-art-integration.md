...
- 当前血量立即更新，刚损失的部分以赭红显示；组件 Inspector 的 `damage_hold_sec` 默认 0.08 秒、`damage_fade_sec` 默认 0.38 秒控制残影收回。连续扣血只维护一个补间，重复数值不重启，恢复血量和上限变化直接同步。
- 新一局与定位清理残影，暂停冻结。数值仍显示真实值（含非致死测试中的负数），条身范围限制为 0～100%。不更改最大魂火、伤害、判定或其他策划规则。
- 绘制使用一个局部 `canvas_item` shader，无纹理模糊、灯光、粒子或额外视口；常态不运行脚本逐帧更新，仍在稳定 HUD 层，不受声波空间折射影响。

验证入口：`tests/visual/run_health_bar_review.gd`（Compatibility）。覆盖首帧、连续扣血、重复通知、暂停、定位、负数、上限变化、独立实例、BOSS 和 720p／1080p／2K／4K，实际教程关截图保存在 `builds/health-bar-review/`。参考构图可见 [《黑神话：悟空》实机截图](https://steamcommunity.com/app/2358720/screenshots/)。
...
主界面选中线位于按钮下方 10 px、线宽 2.4 px；选中美术字用温骨白提亮，非选中保持较暗层次。每次仍只有一个焦点。暂停底框用 AtlasTexture 去除原图非对称透明留白，可见宽度约 768 px，与眼睛及继续按钮共用 x=960 中轴；火根自动跟随可见边缘。三个主按钮约 188×60 px，中心横坐标为 717.24／960／1202.76。框体和按钮以判定中心为基准缩小至上一版的 85%，再上提 25 个设计像素；框体中心 y≈586，收紧下方占用，眼睛与倒计时不缩放、不移动。睁眼独立使用原图圆孔中心约 `(418,169)`、内半径 129 px，对齐设计中心 `(960,540)` 和判定圈 66 px 半径。数字居中，原来的 3、2、1 各一秒不变。暂停主按钮方向左右循环，试玩附加按钮参与 Tab 与向下导航。
...
正式路由仍为 `AppMain → result_screen.tscn`。场景中的 Design 为 1920×1080；中央面板 1440×700，使用结算底图及下一关／重玩／退出切图。眼睛按 stage_id 复用选关目录中的选中态素材，裁去透明留边后在 300×145 区域放大居中；目录之外的关卡沿用通用眼睛。结算标题下不再显示关卡名称。火框复用现行程序燃烧组件，胜利为红焰、失败为蓝焰，随结果立即换色。入场 0.18 秒、离场 0.14 秒，离场期间不重复响应。
