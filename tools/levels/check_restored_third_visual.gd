...
func run() -> void:
	...
	for entry: Dictionary in [{"level":3,"first_judgment_sec":82.0,"notes":36}]:
		...
		var error:=LevelProjectIO.export_zip(workspace.document.data,directory,output.path_join("牲.level.zip"))
