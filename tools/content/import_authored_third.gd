extends SceneTree
## 将关卡编辑器保存的第三关同步到现有内置入口，保持选关与存档 ID。
const DEST := "res://content/stages/level3/"
const PROJECT := "res://content/stage_project/level_3/"
func _initialize() -> void:_run.call_deferred()
func _run() -> void:
	var source:=ProjectSettings.globalize_path("res://../Levels/output/保底关卡/level_3/level.json")
	...
	if opened.level.title!="牲":printerr("来源不是牲，停止导入");quit(1);return
	...
	var previous:=load("res://content/stages/s08/stage_definition.tres") as StageDefinition
	previous.resolve_dependencies_sync()
	stage.stage_id="level3";stage.order_index=3
	stage.reward=previous.reward;stage.unlocked_by_default=false
	stage.reward.next_stage_id=""
	...
	catalog.stages[2]=load(DEST+"stage_definition.tres")
	ResourceSaver.save(catalog,"res://content/catalogs/mvp_catalog.tres")
	print("已更新内置第三关：",stage.display_name,"，ID=",stage.stage_id,"，结果=",error_string(code))
